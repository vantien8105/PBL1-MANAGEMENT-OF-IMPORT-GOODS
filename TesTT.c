...
#include <stdlib.h>
#include <string.h>
//Nhap du lieu vao ban phim thi file tu dong them vao
//Lam bang ngay/ thanh tien/ chi phi tiet kiem
struct Element
{
...
int main(){
	...
	while(toggle_stop){
		...
		printf("-");
		printf("+\n");
			printf("| %-70s|\n","1. Menu thong tin hang hoa"); //done
			printf("| %-70s|\n","2. Menu quan ly thong tin"); // done
			printf("| %-70s|\n","3. Menu tien ich"); //done
			printf("| %-70s|\n","0. Thoat chuong trinh");// done
		printf("+");
		for(i=0;i<71;i++) printf("-");printf("+\n\n");
		int sec;
		printf("Moi ban nhap lua chon: "); scanf("%d", &sec);
		switch(sec){
			case 1:
				{
					system("cls");
					...
					printf("-");
					printf("+\n");
					printf("| %-70s|\n","1. In ra danh sach hang hoa"); //done
					printf("| %-70s|\n","2. In danh sach hang hoa ra file"); // done
					printf("+");
					for(i=0;i<71;i++) printf("-");printf("+\n\n");
					int type;
					printf("Moi ban nhap lua chon: "); scanf("%d", &type);
					switch(type){
						case 1:{
							system("cls");
			...
			case 3:
				{
					...
					printf("+\n");
					printf("| %-70s|\n","1. Tim kiem"); //done
					printf("| %-70s|\n","2. Dieu chinh so luong/ don gia"); // done
					printf("| %-70s|\n","3. Tinh thanh tien cua 1 ngay"); //done
...
void docfile(list main_list){
	...
	char line[100 + 1];
//	fgetc(fi);
	while (fgets(line, sizeof(line), fi) != NULL) { // tsin is FILE* input
		if (line[0] == '\n') {
        	break; // B? qua d?ng tr?ng
   		 }
